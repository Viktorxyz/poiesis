...
`reconcile` is mechanical, not model-driven. It does not infer the integration branch, the remote, the HEAD, or the target — every value is a caller-supplied binding, validated against a re-captured ground truth immediately before the first destructive action. An operation that cannot prove ownership of a binding refuses with a typed error (`RECONCILE_INTEGRATION_BRANCH_MISMATCH`, `RECONCILE_HEAD_MISMATCH`, `RECONCILE_TARGET_MISMATCH`, `RECONCILE_FINGERPRINT_MISMATCH`, `RECONCILE_AUTHORIZATION_MISSING`, `RECONCILE_REMOTE_DRIFT`, `RECONCILE_EXCLUSION_COLLISION`, `RECONCILE_TARGET_COLLISION`, `RECONCILE_WORKTREE_DRIFT`, `RECONCILE_SCAN_INCOMPLETE`, `RECONCILE_NESTED_REPOSITORY`, `RECONCILE_FILTER_ACTIVE`, `RECONCILE_NOT_PRIMARY_CHECKOUT`, …); it never falls back to a best-effort guess, never prunes branches, never deletes remotes, never cleans orphan markers, and never rewrites history.

#### Primary-checkout contract: `git init --separate-git-dir` is unsupported

`reconcile` targets the flat primary-checkout shape where `<root>/.git` is the git directory itself. A repository initialized with `git init --separate-git-dir=<dir>` is a different shape: the worktree contains a `.git` **file** whose contents are `gitdir: <path>` and the actual git data lives at the external `<dir>`. The `git rev-parse --git-dir` and `--git-common-dir` queries still resolve correctly through the pointer, so the existing primary-checkout gate would otherwise pass; but every filesystem read of `<root>/.git/...` (the index, the `sharedindex.*` files, the `info/` directory, the worktree `gitdir` entries, the preserved markers and receipts that live under `poiesis-workspaces-v1` and `poiesis-receipts-v1`) would silently target the wrong path. The fingerprint would miss the shared commondir pointer, the worktree metadata, and every per-checkout resource that lives under the git directory; the destructive `git reset --hard` would still run, but the resulting working tree could not be reconciled to the exact target because the postcondition's index, working-tree, residue, and preserved-shared-path checks would all read from the wrong location.

The `assertNoSeparateGitDir` gate refuses such setups up front with the typed `RECONCILE_NOT_PRIMARY_CHECKOUT` error so callers do not have to read the docs to discover the support boundary. The gate runs before any fingerprint capture, network fetch, or mutation; a separate-git-dir setup never burns cycles on a scan that cannot succeed. The rejection message identifies the layout explicitly:

> `Refusing to reconcile a worktree where `.git` is not a directory (`git init --separate-git-dir` is unsupported)`

To recover from an existing separate-git-dir setup, re-initialize the repository in the canonical primary-checkout shape: back up the worktree contents, remove the external git directory, and run `git init --initial-branch=<branch>` inside the worktree (then restore the contents, re-add the remote, and `git fetch`). The `poiesis init` flow does this automatically — if `init` detects the separate-git-dir shape, it offers to re-initialize before installing any Poiesis-owned state.

A symlinked `<root>/.git` that points at a real directory is NOT refused: that shape is functionally equivalent to having the directory directly and the canonical primary-checkout gate accepts it. The `stat` (not `lstat`) probe follows the symlink so a `.git` symlink that targets a directory surfaces as `isDirectory() === true` and is allowed.
